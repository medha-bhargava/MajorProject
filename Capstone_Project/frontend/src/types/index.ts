...
export interface PurchaseOrder { id: string; sku: string; itemName: string; quantity: number; supplierId: string; requestedBy: string; status: string; unitCost: number; createdAt: string; }
export interface Shipment { id: string; trackingNumber: string; carrier: string; originWarehouse: string; destinationWarehouse: string; sku?: string; quantity?: number; orderId?: string; status: string; shippedAt?: string; deliveredAt?: string; createdAt?: string; }
