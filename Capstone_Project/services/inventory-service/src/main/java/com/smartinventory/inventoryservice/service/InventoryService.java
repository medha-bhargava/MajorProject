...
public class InventoryService {
 ...
 @RabbitListener(queues="inventory.shipment.delivered")
 @Transactional
 public void handleShipmentDelivered(Map<String,Object> event){
  Object sku=event.get("sku");
  Object qty=event.get("quantity");
  if(sku==null||qty==null){return;}
  int quantity=Integer.parseInt(String.valueOf(qty));
  items.findAll().stream()
   .filter(i -> i.getSku().equals(String.valueOf(sku)))
   .findFirst()
   .ifPresent(item -> {
    item.adjust(quantity);
    movements.save(new StockMovement(item.getId(), quantity, "SHIPMENT_DELIVERED"));
...
//  @Transactional public InventoryItemResponse adjust(UUID id,StockAdjustmentRequest request){InventoryItem item=items.findById(id).orElseThrow(() -> new IllegalArgumentException("Inventory item not found")); item.adjust(request.quantityDelta()); movements.save(new StockMovement(id,request.quantityDelta(),request.reason())); publishLowStockIfNeeded(item); return toResponse(item);} 
//  @Transactional public void delete(UUID id){items.deleteById(id);} 
//  @RabbitListener(queues="inventory.procurement.completed") public void handleProcurementCompleted(Map<String,Object> event){ /* Stock is received into inventory when the linked shipment is delivered. */ }
//  @RabbitListener(queues="inventory.shipment.delivered") @Transactional public void handleShipmentDelivered(Map<String,Object> event){Object sku=event.get("sku"); Object qty=event.get("quantity"); if(sku==null||qty==null){return;} int quantity=Integer.parseInt(String.valueOf(qty)); items.findAll().stream().filter(i -> i.getSku().equals(String.valueOf(sku))).findFirst().ifPresent(item -> { item.adjust(quantity); movements.save(new StockMovement(item.getId(), quantity, "SHIPMENT_DELIVERED")); publishLowStockIfNeeded(item); });}
//  private void publishLowStockIfNeeded(InventoryItem item){ if(item.isLowStock()){ rabbitTemplate.convertAndSend(RabbitConfig.EXCHANGE,"inventory.low_stock_detected",Map.of("itemId",item.getId().toString(),"sku",item.getSku(),"name",item.getName(),"quantity",item.getQuantity(),"threshold",item.getLowStockThreshold())); } }
//  private InventoryItemResponse toResponse(InventoryItem i){return new InventoryItemResponse(i.getId(),i.getSku(),i.getName(),i.getCategory(),i.getWarehouseCode(),i.getQuantity(),i.getLowStockThreshold(),i.getUnitCost(),i.getValuation(),i.isLowStock(),i.getUpdatedAt());}
// }
